...
void Init_BuzzerPitch(void)
{
	...
	TIM4->DIER|=TIM_DIER_UIE;						//timer uptdate interrupt enabled
																			//APB clock is Fcy/2 = 180MHz/2 = 90MHz
	TIM4->PSC=PSCArray[BuzzerPSCValue]-1;										//divide APB clock by 256 = 90MHz/256 = 351kHz
	TIM4->ARR=ARRArray[BuzzerARRValue];										//counter reload value, gives a timer period of 100ms when F_APB = 90MHz and PSC = 256
